...
export const saveUser = createAsyncThunk('usersApp/users/saveUser', async (form, { dispatch, getState }) => {
	...
	await axios
		...
		.then(() => {
			axios.post(`${HEROKU_API_URL}/email/sendMailOverHTTP`, mailForm).then(() => {
				dispatch(showMessage({ message: 'Invitation sent to email. Please check the email!', variant: 'success' }));
			});
			dispatch(getUsers());
		})
		.catch(err => {
...
export const updateUser = createAsyncThunk('usersApp/users/updateUser', async (form, { dispatch, getState }) => {
	await axios
		...
		.then(() => dispatch(showMessage({ message: 'User data updated', variant: 'success' })));
