...
export const getMessages = createAsyncThunk(
  "productApp/messages/getMessages",
  async (params) => {
...
export const removeMessages = createAsyncThunk(
  ...
  async ({ messageIds, params }, { dispatch, getState }) => {
    console.log(messageIds, params);
    const response = await axios.post("/api/product-app/remove-messages", {
      messageIds,
    });
    const data = await response.data;
...
} = messagesAdapter.getSelectors((state) => state.productApp.messages);
