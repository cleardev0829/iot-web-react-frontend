...
function DetailSidebarHeader(props) {
  ...
  const selectedItem = useSelector((state) =>
    selectFileById(state, state.productApp.files.selectedItemId)
  );
            ...
            <DialogActions>
              ...
              <Button
                onClick={() =>
                  dispatch(
                    deleteFile({
                      containerName: "rocket-iot-files",
  ...
  const onFileDownload = () => {
    dispatch(
      downloadFile({ containerName: "rocket-iot-files", file: selectedItem })
    ).then(() => {
      dispatch(
        showMessage({
        ...
        <div className="p-12">
          ...
          <FuseAnimate delay={300}>
            <Typography variant="caption" className="">
              <span>Edited</span>
              <span>
                : {moment(selectedItem.modified).format("MMM M, YYYY")}
              </span>
            </Typography>
          </FuseAnimate>
        </div>
