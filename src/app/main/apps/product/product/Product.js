...
import ReactPlaceAutoComplete from 'app/utils/components/ReactPlaceAutoComplete';
import Map from './Map';
...
function Product() {
	...
	function canBeSubmitted() {
		return form.name.length > 0 && !_.isEmpty(form.location) && !_.isEqual(product, form);
	}
									...
									<div className="flex flex-col min-w-0 mx-8 sm:mc-16">
										<FuseAnimate animation="transition.slideLeftIn" delay={300}>
											<Typography className="text-16 sm:text-20 truncate">
												{form.name ? `${form.name} - ${form.uid}` : 'New Lift'}
											</Typography>
										</FuseAnimate>
										<FuseAnimate animation="transition.slideLeftIn" delay={300}>
