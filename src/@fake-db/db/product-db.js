...
mock.onGet(`/api/product-app/products-by-user-id`).reply((request) => {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(`${API_URL}/products/getByUserId`, { params: { userId } })
      .then((response) => {
        const data = response.data;
        resolve([200, data]);
      });
...
mock.onGet("/api/product-app/messages").reply((request) => {
  return new Promise((resolve, reject) => {
    axios
      .get(`${API_URL}/messages/getByPagenation`, {
      ...
      .then((response) => {
        ...
        response.data.map((item) => {
          const log =
            item.hasOwnProperty("message") && item.message.hasOwnProperty("log")
              ? item.message.log
              : "";
          const state =
            item.hasOwnProperty("message") &&
            item.message.hasOwnProperty("state")
              ? item.message.state
              : 0;
          const errid =
            item.hasOwnProperty("message") &&
            item.message.hasOwnProperty("errid")
              ? item.message.errid
              : 0;
...
mock.onGet("/api/product-app/last-messages").reply((request) => {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(`${API_URL}/messages/getLastMsgs`, { params: { limit, skip, log } })
      .then((response) => {
        let data = [];
        response.data.map((item) => {
          const log =
...
mock.onPost("/api/product-app/remove-messages").reply((request) => {
  const { messageIds } = JSON.parse(request.data);

  return new Promise((resolve, reject) => {
    axios
      .post(`${API_URL}/messages/deleteByIds`, { messageIds })
      .then((response) => {
        const data = response.data;
        resolve([200, data]);
      });
...
mock.onPost("/api/product-app/remove-orders").reply((request) => {
  ...
  productDB.orders = productDB.orders.filter(
